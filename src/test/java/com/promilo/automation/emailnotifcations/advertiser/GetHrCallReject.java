...
import com.promilo.automation.pageobjects.emailnotifications.advertiser.gethrcall.RejectedNotification;
...
public class GetHrCallReject  extends BaseClass{
	 ...
	 private static final Logger logger = LogManager.getLogger(RegisteredUserShortList.class);
	    ...
	    private void applyJobDetailsFlow(Page page, String name, String otp, String mailphone) throws Exception {
	        ...
	        approvePage.locator("//button[contains(text(),'Reject')]").click();
	        ...
	        Locator firstEmail = page1.locator("//p[text()='Important Update: HR Call Request Status']").first();
	        ...
	        RejectedNotification rejected= new RejectedNotification(page1);
	        
	        rejected.greetingTex().textContent();
	        rejected.rejectedText().textContent();
	        rejected.exploreText().textContent();
	        
	        rejected.candidateDetails().textContent();
	        
	        
	        rejected.regards().textContent();
	        
	 
	        rejected.myProspect().click();
