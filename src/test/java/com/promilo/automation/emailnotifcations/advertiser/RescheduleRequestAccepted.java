...
public class RescheduleRequestAccepted extends BaseClass {
   ...
   private static final Logger logger = LogManager.getLogger(RescheduleRequestAccepted.class);
   ...
   public void applyForJobAsRegisteredUser(Page page, String inputvalue, String password, String name, String otp, String mailphone) throws Exception {
       ...
       page.locator("//button[@class='functional_btn-Apply-now']").first().click();
    ...
    page.locator("//button[text()='Submit']").nth(1).click(); 
      ...
      Thread.sleep(5000);
       ...
       page3.getByText("Your rescheduled request has been sent to the user.Once the user confirms the").click();
    ...
    // Click on "Review" button and wait for the new page
       Page newPage = page1.waitForPopup(() -> {
           page1.locator("//span[contains(text(),'Review')]").click();
       });

       // Now work on the new page (instead of old page1)
       newPage.locator("img.rescheduled-notification-icon").click();

       // Select the first available time slot
       newPage.locator("//div[@class='rescheduledSlots-time w-100 font-12 text-center mb-50 ']")
              .first()
              .click();

       // Click on "Accept Request" button
       newPage.locator("//button[text()='Accept Request']").click();

       // Get modal text
       String modalText = newPage.locator("//p[text()='Your meeting has been successfully Rescheduled.']").textContent();
