...
public class WhenAdvertiserAcceptsCallBackRequest extends BaseClass {
    ...
    public void CallbackOrTalktoExpertApprovefunctionalityTest() throws InterruptedException, IOException {
        ...
        for (int i = 1; i < rowCount; i++) {
            ...
            try {
                ...
                page.locator("//p[contains(text(),'Confirmation: Your Expert Talk with ')]").first().click();

                page.locator("//span[contains(text(),'Hi')]").textContent();
                page.locator("//p[contains(text(),' Your talk to expert request with ')]").textContent();
                page.locator("//p[contains(text(),'Expert will reach out soon to answer all your queries.')]").textContent();
              Locator card = page.locator("//tbody/tr/td[@align='left']/a/div[@align='center']/table[1]");
              card.textContent();

         Locator    newPage=   page.locator("//span[contains(text(),'Click')]");
        		 newPage.click();
