...
public class WhenUserAcceptsRecruitersRescheduleRequest extends BaseClass {

    ExtentReports extent = ExtentManager.getInstance();
    // Test Description
    ExtentTest test = extent.createTest("🚀 promilo shortlist functionality with email notification validation");

    private static final Logger logger = LogManager.getLogger(WhenUserAcceptsRecruitersRescheduleRequest.class);
...
page3.getByText("Your rescheduled request has been sent to the user.Once the user confirms the")
     ...
     // Open a new tab explicitly
        Page page4 = page.context().newPage();
        ...
        page4.locator("//p[contains(text(),'Reschedule Request Accepted for')]")
                .first().click();
        page4.locator("//span[contains(text(),'Greetings')]").textContent();
        page4.locator("//p[contains(text(),'Great news! Your confirmation')]").isVisible();
        page4.locator("//td[@style='padding-left: 10px']").textContent();
        page4.locator("//span[contains(text(),'My Meetings')]").click();
