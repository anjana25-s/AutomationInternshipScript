...
public class GuestUserFillApplication extends BaseClass {
    ...
    public void applyForJobWithInvalidData() throws Exception {
     ...
     // ======== Load Excel ========
        ...
        page.waitForTimeout(2000);         
                ...
                fillApplication.paymentSuccessButton(popup2).click();
                ...
                String successText = fillApplication.paymentSuccessMessage()
                        .textContent()
                        .replace("\n", " ").trim();

                Assert.assertEquals(successText, expectedMessage);

                fillApplication.myApplicationLink().click();
                ...
                String campaignTitle = fillApplication.campaignName().textContent().trim();
                Assert.assertEquals(campaignTitle, "Golden Institute of Technology", "Institute name is not matching!");

                String location = fillApplication.location().textContent().trim();
                Assert.assertEquals(location, "Nandyal", "Location is not matching!");

                String serviceName = fillApplication.serviceName().textContent().trim();
                Assert.assertEquals(serviceName, "Application", "Service name is not matching!");

                Assert.assertTrue(fillApplication.serviceIcon().isVisible(), "Service icon is not visible!");
