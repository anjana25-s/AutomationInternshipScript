...
public class GetHrCallRequestRejected  extends BaseClass{
	 ...
	 private static final Logger logger = LogManager.getLogger(RegisteredUserShortList.class);
	    ...
	    private void applyJobDetailsFlow(Page page, String name, String otp, String mailphone) throws Exception {
	        ...
	        Locator rejectBtn = approvePage.locator("[class='modal-content']");
	        rejectBtn.textContent();
	        
	        approvePage.locator("//button[@class='btn done-btn w-100']").click();

	     // Wait until visible
	     rejectBtn.waitFor(new Locator.WaitForOptions().setState(WaitForSelectorState.VISIBLE));

	     // Assertion before click
	     Assert.assertTrue(rejectBtn.isVisible(), "❌ Reject button not visible!");
