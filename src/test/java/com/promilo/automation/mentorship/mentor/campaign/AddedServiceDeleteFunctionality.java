        ...
        for (int i = 1; i <= lastRow; i++) {
            ...
            try {
             ...
             // List of service labels
                ...
                // Delete all cards using your same locator logic
                for (String s : servicesValidation) {

                    // Check if card is present before clicking
                    if (page.locator("//p[text()='" + s + "']").count() > 0) {

                        // Click close icon (your original XPath)
                        page.locator("//p[text()='" + s + "']//following::img[@alt='close']")
                                .first().click();

                        // Click Remove
                        page.locator("//span[text()='Remove']").click();

                        page.waitForTimeout(500);
                    }
                }

                // After loop ends → print final message
                System.out.println("🎉 All cards are deleted");
