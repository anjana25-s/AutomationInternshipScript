...
public class TalkToExperts extends Baseclass {
    ...
    public void TaltoExpert() throws InterruptedException, IOException {
        ...
        for (int i = 1; i < rowCount; i++) {
            ...
            page.locator("//button[text()='Talk to Experts']").first().click();
            ...
            page.locator("//input[@type='text']").nth(1).fill("1. Which Course are you looking for?");
            page.locator("//input[@type='text']").nth(1).fill("Bangalore");
            ...
            test.pass("🎉 Job applied successfully — Popup: " + popupText);
                      ...
                      ExcelReadUtil talkToExpertExcel = new ExcelReadUtil(excelPath, "TalkToExperts");

            int lastRow = talkToExpertExcel.getRowCount();

            

            // Append current run data
            talkToExpertExcel.setCellData(lastRow, 6, "karthik");
            talkToExpertExcel.setCellData(lastRow, 1, randomPhone);
            talkToExpertExcel.setCellData(lastRow, 2, randomEmail);
            talkToExpertExcel.setCellData(lastRow, 3, otp);
            talkToExpertExcel.setCellData(lastRow, 4, "1. Which Course are you looking for?");
            talkToExpertExcel.setCellData(lastRow, 5, "Bangalore");
            talkToExpertExcel.setCellData(lastRow, 0, this.getClass().getSimpleName());
