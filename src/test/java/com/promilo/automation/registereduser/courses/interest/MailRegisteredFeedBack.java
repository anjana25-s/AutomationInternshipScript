...
public class MailRegisteredFeedBack extends BaseClass {
    ...
    public void applyForJobWithInvalidData() throws Exception {
     ...
     // ======== Load Excel ========
        ...
        test.info("Logged in as registered user: " + registeredEmail);
            ...
            test.info("📚 Clicked on Courses tab.");
            Thread.sleep(2000);
            ...
            page.locator("//textarea[@placeholder='Write a feedback']").fill("something");
            test.info("📝 Entered feedback text: 'something'");
            page.locator("//button[text()='Submit']").click();
            test.info("📤 Submitted feedback form.");
            ...
            test.info("📤 Submitted user details for feedback.");
            ...
            notification.click();
            test.info("🔔 Opened in-app notifications.");

            String notifText = page.locator("//h6[contains(text(),'Thank You for Your Feedback! We’ve received your feedback ')]").textContent();
            test.info("📩 Notification validated: " + notifText);
            ...
            test.info("✉️ Entered Mailosaur email.");
            approveMail.MailsaurContinue();
            test.info("➡️ Clicked Continue.");
            approveMail.MailsaurPassword();
            test.info("🔑 Entered Mailosaur password.");
            approveMail.MailsaurLogin();
            test.info("🔓 Logged into Mailosaur.");

            page.locator("//p[text()='Thank You for Your Valuable Feedback! 🎉']").first().click();
            test.info("📩 Opened feedback confirmation email.");

            page.locator("//span[contains(text(),'Hi ')]").textContent();
            page.locator("//p[contains(text(),'Thank you for sharing your')]").textContent();
            page.locator("//strong[contains(text(),'What’s')]").textContent();
            page.locator("//p[contains(text(),'We')]").textContent();
            Locator table = page.locator("//p[contains(text(),'In the meantime, keep')]");
            test.info("📑 Extracted email content: " + table.textContent());
        }
