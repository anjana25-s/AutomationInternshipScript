...
public class AssignmentWithoutProvidingData extends BaseClass {
    ...
    private static final Logger logger =
            LogManager.getLogger(AssignmentWithoutProvidingData.class);
    ...
    page.locator("//img[@alt='closeIcon Ask us']").first().click();
        ...
        Locator error =
                page.locator("//p[text()='Minimum 3 characters required']");
        error.waitFor(new Locator.WaitForOptions().setTimeout(5000));

        Assert.assertTrue(error.isVisible(),
                "Error message not displayed for empty assignment");

        test.pass("Correct error shown for empty assignment submission");
    }
}
