...
public class GuestUserJobApplyWithInvalidOtp extends BaseClass{
    ...
    public Object[][] jobApplicationData() throws Exception {
        ...
        for (int i = 1; i < rowCount; i++) {
            ...
            data[i - 1][2] = excel.getCellData(i, 2); // FieldType
            data[i - 1][3] = excel.getCellData(i, 3); // InputValue
            data[i - 1][4] = excel.getCellData(i, 4); // ExpectedResult
            data[i - 1][5] = excel.getCellData(i, 5); // OTP
            data[i - 1][6] = excel.getCellData(i, 6); // Password
            data[i - 1][7] = excel.getCellData(i, 7); // Name
            data[i - 1][8] = excel.getCellData(i, 8); // ApplyPhone
        }
        return data;
    }
