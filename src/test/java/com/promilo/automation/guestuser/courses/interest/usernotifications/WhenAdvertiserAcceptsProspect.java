...
// ✅ This ensures FreeVideoCounselling class runs before this class


public class WhenAdvertiserAcceptsProspect extends BaseClass {
	@Test(
		      dependsOnMethods = {
        ...
        for (int i = 1; i < rowCount; i++) {
         ...
         //   Assert.assertTrue(login.signInContent().isVisible(), "❌ Sign-in content not visible.");
          //  Assert.assertTrue(login.talkToAnExpert().isVisible(), "Talk To Expert should be visible");
            ...
            page.locator("//p[contains(text(),'Confirmation of Accepted Meeting Request ')]").first().click();
            page.locator("//span[contains(text(),'Hi')]").textContent();
            page.locator("//a[@style='text-decoration: none;cursor:pointer;']").textContent();
            page.locator("//p[contains(text(),'I trust this email finds you well. I am')]").textContent();
            
            page.locator("//span[text()='Reschedule']").click();
