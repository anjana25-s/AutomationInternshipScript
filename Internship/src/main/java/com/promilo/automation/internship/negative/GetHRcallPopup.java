...
public class GetHRcallPopup {
	private Page page;

public GetHRcallPopup(Page page) {
	   this.page=page;
 }
...
public void verifyIndustryDropdown() {
	Locator dropdownError=page.locator("//div[text()='Industry is required']");
	
	 String actual = dropdownError.textContent().trim();
	    boolean isVisible = dropdownError.isVisible();

	    System.out.println("👉 Industry Error Visible: " + isVisible);
	    System.out.println("📝 Visible Text: " + actual);

	    Assert.assertTrue(isVisible, "❌ Industry error message is not visible!");
	    Assert.assertEquals(actual, "Industry is required", "❌ Incorrect Industry error message!");
	    System.out.println("✅ dropdown validation passed successfully.\n");
}
...
public void screeningQuestion() {
	Locator screeningQuestionError=page.locator("//div[text()='Answer required questions']");
	screeningQuestionError.waitFor(new Locator.WaitForOptions().setTimeout(5000));
	
	String actual = screeningQuestionError.textContent().trim();
    boolean isVisible = screeningQuestionError.isVisible();

    System.out.println("👉 error Message Visible: " + isVisible);
    System.out.println("📝 Visible Text: " + actual);

    Assert.assertTrue(isVisible, "❌ error message is not visible!");
    Assert.assertEquals(actual, "Answer required questions", "❌ Incorrect error message!");
    System.out.println("✅ error message validation passed.\n");
}

public void alreadyShownInterest() {
	Locator alreadyShownInterestError=page.locator("//div[text()='You already shown interest on this campaign.']");
	alreadyShownInterestError.waitFor(new Locator.WaitForOptions().setTimeout(6000));
	
	String actual = alreadyShownInterestError.textContent().trim();
    boolean isVisible = alreadyShownInterestError.isVisible();

    System.out.println("👉 error Message Visible: " + isVisible);
    System.out.println("📝 Visible Text: " + actual);

    Assert.assertTrue(isVisible, "❌ error message is not visible!");
    Assert.assertEquals(actual, "You already shown interest on this campaign.", "❌ Incorrect error message!");
    System.out.println("✅ error message validation passed.\n");
}
