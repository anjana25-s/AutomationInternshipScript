...
public class GetHrCallWithInvalidData extends BaseClass {
    ...
    ExtentTest test = extent.createTest("🚀 Promilo Staging Signup - HR Call with Invalid Data (Playwright)");

    private static final Logger logger = LogManager.getLogger(GetHrCallWithInvalidData.class);
    ...
    public void applyForJobTestFromExcel(
        ...
        ExtentReports extent = ExtentManager.getInstance();
        ExtentTest test = extent.createTest("Apply for Job (Invalid Data) | " + testCaseId);
        ...
        test.pass("✅ Invalid Data Job application test executed for TestCase: " + testCaseId);
        extent.flush();
    }
    ...
    private void applyJobDetailsFlow(Page page, String name, String mailphone) throws Exception {
        ...
        // Invalid name & mobile
        page.locator("//div[@class='ask-us-popup-form-side']//input[@id='userName']").fill("karthik123");
        test.info("Filled invalid user name");

        page.locator("//div[@class='ask-us-popup-form-side']//input[@id='userMobile']").fill("111");
        test.info("Filled invalid mobile number");
        ...
        test.info("Clicked on Get An HR Call with invalid details");
    }
}
