...
import com.promilo.automation.courses.intrestspages.CampusVisit;
...
public class MailRegisteredCampusVisit   extends BaseClass{
    ...
    public void applyForJobWithInvalidData() throws Exception {
     ...
     // ======== Load Excel ========
        ...
        page.locator("//input[@placeholder='Search Colleges and Courses']").fill("fill");
        ...
        CampusVisit CampusVisitComponents= new CampusVisit(page);
     ...
     // ✅ Invalid inputs validation
        ...
        String currentMonth = CampusVisitComponents.currentMonth().textContent().trim();
        ...
        Locator dateElement = CampusVisitComponents.availableDate();
        dateElement.click();
        BaseClass.selectedDate = dateElement.innerText().split("\\s+")[0].trim();

        Locator timeElement = CampusVisitComponents.firstTimeSlot();
        timeElement.click();
        ...
        CampusVisitComponents.thankYouPopUpExitIcon().click();
        CampusVisitComponents.visitTab().click();
        
        String campaignName=CampusVisitComponents.campaignName().textContent().trim();
        assertEquals(campaignName, "Application form fill mobile brand name");
        
        
        String location= CampusVisitComponents.location().textContent().trim();
        assertEquals(location, "Kerala - Other");
        
        String serviceName= CampusVisitComponents.serviceName().textContent().trim();
        assertEquals(serviceName, "Campus Visit");
        
        CampusVisitComponents.serviceIcon().isVisible();
        
        
        String statusTag= CampusVisitComponents.statusTag().textContent().trim();
        assertEquals(statusTag, "Requested");
        
        
        
        String meetingDate = CampusVisitComponents.visitingDate().first().textContent().trim();
        ...
        String meetingTime = CampusVisitComponents.visitingTime().first().textContent().trim();
